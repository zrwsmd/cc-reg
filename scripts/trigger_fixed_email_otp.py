...
from src.config.constants import OPENAI_PAGE_TYPES, EmailServiceType
...
def main() -> int:
    ...
    if not did:
        print("[脚本] 失败：没有拿到 Device ID")
        return 1
    if not sen_token:
        print("[脚本] 警告：没有拿到 Sentinel token，继续尝试")

    signup_result = engine._submit_signup_form(did, sen_token)
    if not signup_result.success:
        print(f"[脚本] 失败：注册入口没有走通：{signup_result.error_message}")
        return 2
    ...
    if signup_result.page_type == OPENAI_PAGE_TYPES["EMAIL_OTP_VERIFICATION"]:
        print("[脚本] 当前邮箱直接进入验证码页，OpenAI 可能已经自动发送验证码")
    ...
    if signup_result.page_type != OPENAI_PAGE_TYPES["PASSWORD_REGISTRATION"]:
        print(f"[脚本] 失败：当前页面不是 create_account_password，而是 {signup_result.page_type or 'unknown'}")
        return 4

    password_ok, password = engine._register_password(did, sen_token)
    if not password_ok:
        print(f"[脚本] 失败：提交密码没有走通：{engine._last_register_password_error or 'unknown'}")
        return 5

    print(f"[脚本] 本次写死邮箱生成密码: {password}")
